...
@pushOnce('scripts')
    <script type="text/x-template" id="v-historial-ia-person-template">
    ...
    <script type="module">
        app.component('v-historial-ia-person', {
            template: '#v-historial-ia-person-template',
            ...
            computed: {
                personName() {
                    return this.person?.name ?? 'Persona';
                },

                personEmail() {
                    return this.person?.email ?? null;
                },
                    ...
                    for (let i = 0; i < msgs.length; i++) {
                        ...
                        if (t === 'human') {
                            if (pending && !pending.user) {
                                ...
                                pending = { user: item, ia: null };
                                if (msgs[i + 1] && (msgs[i + 1].type ?? this.detectType(msgs[i + 1].raw ?? '')) === 'ai') {
                                    pending.ia = msgs[i + 1];
                                    rows.push(pending);
                                    pending = null;
                                    i++;
                            ...
                            if (pending && !pending.ia) {
                                ...
                                pending = { user: null, ia: item };
                                if (msgs[i + 1] && (msgs[i + 1].type ?? this.detectType(msgs[i + 1].raw ?? '')) === 'human') {
            ...
            methods: {
                fetchHistory() {
                    if (!this.personEmail) {
                        this.error = 'No se encontró email de la persona.';
                        return;
                    }
                    ...
                    this.$axios.get(this.endpoint, { params: { email: this.personEmail } })
                        .then(res => {
                ...
                normalizeMessages(rawList) {
                    ...
                    for (const item of rawList) {
                        ...
                        out.push({ raw, fecha, mensaje, type });
                    }
