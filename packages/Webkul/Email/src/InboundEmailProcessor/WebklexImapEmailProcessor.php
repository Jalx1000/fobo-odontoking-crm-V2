...
class WebklexImapEmailProcessor implements InboundEmailProcessor
{
    ...
    public function processMessage($message = null): void
    {
         ...
         * To Do: Review this.
         */
        $rawFolderName = $message->getFolder()->name;
        $segments      = explode('/', $rawFolderName);
        ...
        switch (strtolower($baseName)) {
            case 'inbox':
                ...
                break;
            case 'important':
                $folderName = SupportedFolderEnum::IMPORTANT->value;
                break;
            case 'starred':
                $folderName = SupportedFolderEnum::STARRED->value;
                break;
            case 'drafts':
            case 'draft':
                $folderName = SupportedFolderEnum::DRAFT->value;
                break;
            case 'sent mail':
            case 'sent':
            case 'sent items':
                $folderName = SupportedFolderEnum::SENT->value;
                break;
            case 'trash':
            case 'deleted items':
            case 'bin':
                $folderName = SupportedFolderEnum::TRASH->value;
                break;
            default:
                $folderName = '';
        }
        ...
        if ($email) {
            $parentEmail = $this->emailRepository->update([
                ...
                'reference_ids' => array_unique(array_merge($email->reference_ids ?? [], $references)),
            ], $email->id);
        }
    ...
    protected function processMessagesFromLeafFolders($rootFoldersCollection = null): void
    {
        $rootFoldersCollection->each(function ($folder) {
            ...
            return $folder->query()->since(now()->subDays(90))->get()->each(function ($message) {
