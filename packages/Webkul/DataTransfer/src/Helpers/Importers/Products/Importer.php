...
class Importer extends AbstractImporter
{
    ...
    protected function saveProductsData(ImportBatchContract $batch): bool
    {
        ...
        if (! empty($products['insert'])) {
            $newSkus = array_keys($products['insert']);

            $newProducts = $this->productRepository->findWhereIn('sku', $newSkus, ['id', 'sku']);
    ...
    /**
     * Prepare attribute values from current row.
     */
    public function prepareAttributeValues(array $rowData, array &$attributeValues): void
    {
        $typeFields = $this->productRepository->getModel()::$attributeTypeFields;
    ...
    /**
     * Save attribute values for products.
     */
    public function saveAttributeValues(array $attributeValues): void
    {
        $productAttributeValues = [];

        foreach ($attributeValues as $sku => $values) {
            ...
            foreach ($values as $valueRow) {
                $valueRow['entity_id']   = (int) $product['id'];
                $valueRow['entity_type'] = 'products';
                ...
                $valueRow['unique_id'] = $uniqueId;

                $productAttributeValues[$uniqueId] = $valueRow;
            }
        }

        if (! empty($productAttributeValues)) {
            $this->attributeValueRepository->upsert($productAttributeValues, 'unique_id');
        }
    }
