...
class ActivityRepository extends Repository
{
    ...
    public function create(array $data)
    {
        ...
        foreach ($data['participants']['doctors'] ?? [] as $doctorId) {
            if (empty($doctorId)) {
                continue;
            }

            $activity->doctors()->attach($doctorId);
        }
    ...
    public function update(array $data, $id, $attribute = 'id')
    {
        ...
        if (isset($data['participants'])) {
            ...
            if (isset($data['participants']['doctors'])) {
                $doctorIds = array_filter($data['participants']['doctors'] ?? [], fn ($id) => ! empty($id));
                $activity->doctors()->sync($doctorIds);
            } else {
    ...
    public function isDurationOverlapping($startFrom, $endFrom, $participants, $id)
    {
        $queryBuilder = $this->leftJoin('activity_participants', 'activities.id', '=', 'activity_participants.activity_id')
            ...
            ->where(function ($query) use ($participants) {
                ...
                if (isset($participants['doctors'])) {
                    $query->orWhereIn('doctor_activities.doctor_id', $participants['doctors']);
                }
            })
